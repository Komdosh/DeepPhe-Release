...
public class Sectionizer extends JCasAnnotator_ImplBase {
            ...
            while ((line = br.readLine()) != null) {
                if (!line.trim().startsWith(PARAM_COMMENT)) {
                    ...
                    if (fields != null && fields.length > 1 && fields[0] != null
                        ...
                        if (oldPattern != null)
                            LOGGER.info("WARNING: already had a pattern '" + oldPattern + "' for " + id + " which was just replaced by " + p);

                        if (fields.length > 1 && fields[1] != null) {
                            String temp = fields[1].trim();
                            String previous = section_names.put(id, temp);
                            if (previous != null)
                                LOGGER.info("WARNING: already had a section '" + previous + "' for " + id + " which was just replaced by " + temp);
    ...
    private void addSingleSectionToCas(JCas cas) {
...
//                LOGGER.info( "Adding " + segment.getPreferredText() + " " + segment.getId() + " " + segment.getTagText() );
    ...
    public void process(JCas jCas) throws AnalysisEngineProcessException {
        ...
        if (text == null) {
            String docId = new DocumentID(jCas).getDocumentID();
